...
import {
  FiArrowLeft, FiPlus, FiUsers, FiShare2, FiCopy,
  FiEdit2, FiTrash2, FiDollarSign, FiCheckCircle,
  FiXCircle, FiActivity, FiCreditCard, FiShoppingCart,
  FiX, FiCheck, FiSend, FiMessageCircle,
} from "react-icons/fi";
...
export default function GroupDetail() {
  ...
  const copyLink = () => { navigator.clipboard.writeText(inviteLink); toast.success("Link copied!"); };
  ...
  const handleUpdateSettlement = async () => {
    ...
    try {
      await updateSettlement(settlementModal._id, { amount, note: settlementModal.note, relatedExpenses: settlementExpenseIds.length > 0 ? settlementExpenseIds : null });
      toast.success("Payment updated!");
      setSettlementModal(null); setSettlementAmount(""); setSettlementExpenseIds([]);
      fetchAll();
    } catch (err) { toast.error(err.response?.data?.msg || "Failed to update payment"); }
  ...
  const handleDeleteSettlement = async () => {
    ...
    try {
      ...
      setSettlementModal(null); setSettlementAmount(""); setSettlementExpenseIds([]);
      fetchAll();
    } catch (err) { toast.error(err.response?.data?.msg || "Failed to delete payment"); }
    finally { setSavingSettlement(false); }
  };

  const shareOnWhatsApp = () => {
    window.open("https://wa.me/?text=" + encodeURIComponent("Hey! Join my group on Splitwise Clone!\nClick here to join: " + inviteLink), "_blank");
  };

  const getItemDate  = (item) => new Date(item.date || item.createdAt || item.updatedAt || Date.now());

  const activityItems = [
    ...expenses.map(e  => ({ type: "expense",    id: `expense-${e._id}`,    date: getItemDate(e),  data: e  })),
    ...(settlementSummary.settlements || []).filter(s => !s.isExpenseUpdate)
      .map(s => ({ type: "settlement", id: `settlement-${s._id}`, date: getItemDate(s), data: s })),
  ].sort((a, b) => b.date - a.date);

  const userDebts     = settlementSummary.userDebts || [];
  const totalUserDebt = userDebts.reduce((t, d) => t + Number(d.amount || 0), 0);
  ...
  const getExpenseOptionsForSettlement = (settlement) => {
    ...
    return expenses.filter(expense => {
      const expPaidById = expense.paidBy?._id || expense.paidBy?.id || expense.paidBy;
      const payerInSplit = expense.splits?.some(split => (split.user?._id || split.user?.id || split.user) === payerId);
...
const groupedMessages = (Array.isArray(chatMessages) ? chatMessages : []).reduce((acc, msg) => {
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="grid grid-cols-4 sm:flex sm:items-center gap-2 w-full sm:w-auto">
          <button onClick={handleInvite} className="flex items-center justify-center gap-1.5 bg-gray-800 hover:bg-gray-700 text-white px-2 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-medium transition">
            <FiShare2 size={14} className="shrink-0" /><span className="truncate">Invite</span>
          </button>
          <button onClick={openWishlistModal} className="flex items-center justify-center gap-1.5 bg-purple-600 hover:bg-purple-700 text-white px-2 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-medium transition">
            <FiShoppingCart size={14} className="shrink-0" /><span className="truncate hidden sm:inline">Wishlist</span><span className="truncate sm:hidden">Import</span>
          </button>
          <Link to={`/group/${groupId}/add-expense`} className="flex items-center justify-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 text-white px-2 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-medium transition">
            <FiPlus className="shrink-0" /><span className="truncate">Add</span>
          </Link>
          <button onClick={() => navigate("/group/" + groupId + "/settle-up")} className="flex items-center justify-center gap-1.5 bg-orange-500 hover:bg-orange-600 text-white px-2 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-medium transition">
            <FiDollarSign size={14} className="shrink-0" /><span className="truncate">Settle</span>
          </button>
        </div>
      </div>
      ...
      {/* ── Tab Content ─────────────────────────────────────────── */}
      <div className={`max-w-2xl mx-auto px-4 sm:px-6 ${activeTab === 'chat' ? 'py-0' : 'py-6'}`}>
        {loading ? (
          ...
          <div className="space-y-3 py-6">
            ...
            {activityItems.length === 0 ? (
              <div className="text-center py-20">
                ...
                <Link to={`/group/${groupId}/add-expense`} className="inline-block mt-4 text-emerald-400 hover:underline text-sm">Add first expense</Link>
              </div>
            ) : activityItems.map(item => {
              if (item.type === "settlement") {
                const s = item.data;
                return (
                  <div key={item.id} onClick={() => openSettlementModal(s)} className="bg-gray-900 border border-emerald-500/30 hover:border-emerald-400 rounded-2xl p-4 cursor-pointer transition">
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {s.relatedExpenses?.length > 0
                            ? <p className="text-xs text-emerald-300 mt-1">For {s.relatedExpenses.map(e => e.description).join(', ')}</p>
                            : s.relatedExpense
                            ? <p className="text-xs text-emerald-300 mt-1">For {s.relatedExpense.description}</p>
                            : null}
                        </div>
                      </div>
                      <span className="text-emerald-400 font-bold text-lg shrink-0">₹{s.amount.toFixed(2)}</span>
                    </div>
                  </div>
                );
              }
                  ...
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-gray-400 text-sm mt-0.5 truncate">
                        Paid by {exp.paidByMultiple?.length > 1 ? `${exp.paidBy?.name || 'Someone'} and ${exp.paidByMultiple.length - 1} other${exp.paidByMultiple.length > 2 ? 's' : ''}` : exp.paidBy?.name || 'Someone'}
                      </p>
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="text-emerald-400 font-bold text-lg">₹{exp.amount}</span>
                      <button onClick={() => navigate("/group/" + groupId + "/edit-expense/" + exp._id)} className="p-1.5 text-gray-400 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg transition"><FiEdit2 size={14} /></button>
                      <button onClick={() => setDeleteModal(exp)} className="p-1.5 text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition"><FiTrash2 size={14} /></button>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {exp.splits?.map(split => {
                      const splitUserId = split.user?._id || split.user;
                      const isPayer = exp.paidByMultiple?.length > 0
          ...
          <div className="space-y-3 py-6">
            {Object.keys(balances).length === 0 ? (
              <div className="text-center py-20"><div className="text-5xl mb-4">✅</div><p className="text-gray-400">All settled up!</p></div>
            ) : (
              <>
                  ...
                  <div className="space-y-3">
                    {settlementSummary.suggestions.map((item, i) => (
                      <div key={i} className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
                        <p className="font-medium text-sm break-words">{item.paidBy.name} borrows ₹{item.amount.toFixed(2)} from {item.paidTo.name}</p>
                        <p className="text-xs text-gray-500 mt-1">{item.paidBy.email} owes {item.paidTo.email}</p>
                      </div>
                    ))}
                  </div>
                )}
                <div className="pt-3 space-y-3">
                  {Object.entries(balances).map(([userId, balance]) => (
                    <div key={userId} className="bg-gray-900 border border-gray-800 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center shrink-0"><FiUsers className="text-gray-400" /></div>
                        <div className="min-w-0">
                      ...
                      {Math.abs(balance.amount) < 0.01
                        ...
                        : <span className={`font-bold text-lg ${balance.amount > 0 ? "text-emerald-400" : "text-red-400"}`}>{balance.amount > 0 ? "+" : "-"}₹{Math.abs(balance.amount).toFixed(2)}</span>
                      }
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-4 py-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gray-800 before:to-transparent">
            {history.length === 0 ? (
              <div className="text-center py-20 relative z-10"><div className="text-5xl mb-4">📝</div><p className="text-gray-400">No activity yet</p></div>
            ) : history.map(item => {
              ...
              return (
                <div key={item._id} className="relative z-10 flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                  <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-gray-950 bg-gray-900 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 mx-auto">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${bgColor}`}><Icon size={14} className={iconColor} /></div>
                  </div>
                ...
                Object.entries(groupedMessages).map(([dateLabel, msgs]) => (
                  <div key={dateLabel}>
                    ...
                    <div className="flex items-center gap-3 my-4">
                      ...
                      <span className="text-xs text-gray-500 bg-gray-900 px-3 py-1 rounded-full border border-gray-800">{dateLabel}</span>
                      <div className="flex-1 h-px bg-gray-800" />
                    </div>
                    ...
                    {msgs.map((msg, idx) => {
                      ...
                      return (
                        <div key={msg._id} className={`flex items-end gap-2 mb-1 group ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                          ...
                          <div className={`max-w-[75%] ${isMe ? 'items-end' : 'items-start'} flex flex-col`}>
                            ...
                            <div className={`relative px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                              ...
                              <p className={`text-xs mt-1 ${isMe ? 'text-emerald-200' : 'text-gray-500'}`}>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-end sm:items-center justify-center z-50 px-4 pb-4 sm:pb-0">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 sm:p-6 w-full max-w-md max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between mb-5 shrink-0">
              <div>
                <h2 className="text-lg font-bold flex items-center gap-2"><FiShoppingCart className="text-purple-400" /> Import from Wishlist</h2>
                <p className="text-xs text-gray-400 mt-0.5">Select an item to add as group expense</p>
              </div>
              <button onClick={() => setWishlistModal(false)} className="text-gray-400 hover:text-white transition"><FiX size={20} /></button>
            </div>

            <div className="overflow-y-auto flex-1 space-y-4 pr-1">
              <div>
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Step 1 — Choose Wishlist Item</p>
                {wishlistLoading ? (
                  ...
                  <div className="text-center py-6 border border-dashed border-gray-700 rounded-xl">
                    <p className="text-4xl mb-2">🛍️</p><p className="text-gray-400 text-sm">No pending wishlist items</p>
                  </div>
                ) : wishlistItems.map(item => {
                  const isSelected = selectedWishItem?._id === item._id;
                  return (
                    <div key={item._id} onClick={() => handleSelectWishItem(item)} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition mb-2 ${isSelected ? 'border-purple-500 bg-purple-500/10' : 'border-gray-700 bg-gray-800 hover:border-gray-600'}`}>
                      <div className="w-9 h-9 rounded-xl bg-gray-700 flex items-center justify-center text-lg shrink-0">{getCatEmoji(item.category)}</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{item.title}</p>
                        <p className="text-xs text-gray-400">by {item.addedBy?.name || 'Someone'}{item.price > 0 && <span className="text-emerald-400 ml-2">₹{item.price.toLocaleString()}</span>}</p>
                      </div>
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition ${isSelected ? 'border-purple-500 bg-purple-500' : 'border-gray-600'}`}>
              ...
              {selectedWishItem && (
                <div>
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Step 2 — Confirm Amount</p>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm">₹</span>
                    <input type="number" min="1" value={wishAmount} onChange={e => setWishAmount(e.target.value)} className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl pl-8 pr-4 py-3 focus:outline-none focus:border-purple-500 transition text-sm" placeholder="Enter amount" />
                  </div>
                  <p className="text-xs text-gray-500 mt-1">Wishlist price: ₹{selectedWishItem.price > 0 ? selectedWishItem.price.toLocaleString() : 'not set'}</p>
                </div>
              )}

              {selectedWishItem && (
                <div>
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Step 3 — Who Paid?</p>
                  <div className="space-y-2">
                    {groupMembers.map(member => {
                      ...
                      return (
                        <div key={memberId} onClick={() => setWishPaidBy(memberId)} className={`flex items-center justify-between gap-3 p-3 rounded-xl border cursor-pointer transition ${selected ? 'border-blue-500 bg-blue-500/10' : 'border-gray-700 bg-gray-800 hover:border-gray-600'}`}>
                          <div className="flex items-center gap-3 min-w-0">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ${selected ? 'bg-blue-500 text-white' : 'bg-gray-700 text-gray-400'}`}>{name[0]?.toUpperCase()}</div>
                            <p className="text-sm font-medium truncate">{name}{isMe && <span className="text-xs text-gray-400 ml-1">(you)</span>}</p>
                          </div>
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition ${selected ? 'border-blue-500 bg-blue-500' : 'border-gray-600'}`}>
                  ...
                  <p className="text-xs text-gray-500 mt-2">💡 Split equally among all {groupMembers.length} members</p>
                </div>
              )}
            </div>

            <div className="flex gap-3 pt-4 shrink-0 border-t border-gray-800 mt-4">
              <button onClick={() => setWishlistModal(false)} className="flex-1 bg-gray-800 hover:bg-gray-700 text-white py-3 rounded-xl transition text-sm">Cancel</button>
              <button onClick={handleImportFromWishlist} disabled={!selectedWishItem || !wishPaidBy || !wishAmount || wishImporting} className="flex-1 bg-purple-600 hover:bg-purple-700 disabled:opacity-40 disabled:cursor-not-allowed text-white py-3 rounded-xl font-semibold transition text-sm">
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 sm:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="text-center mb-6">
              <div className="w-14 h-14 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto mb-4"><FiShare2 className="text-emerald-400" size={24} /></div>
            ...
            <button onClick={copyLink} className="w-full flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white py-3 rounded-xl font-semibold transition mb-3"><FiCopy /> Copy Invite Link</button>
            <button onClick={shareOnWhatsApp} className="w-full flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl font-semibold transition mb-3">📱 Share on WhatsApp</button>
            <button onClick={() => setShowInvite(false)} className="w-full bg-gray-800 hover:bg-gray-700 text-white py-3 rounded-xl transition">Close</button>
          </div>
        </div>
      )}
      ...
      {settlementModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 sm:p-6 w-full max-w-sm">
            <div className="text-center mb-6">
              <div className="w-14 h-14 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto mb-4"><FiDollarSign className="text-emerald-400" size={24} /></div>
            ...
            <div className="mb-5">
              ...
              <input type="number" min="1" value={settlementAmount} onChange={e => setSettlementAmount(e.target.value)} className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500 transition" autoFocus />
            </div>
            <div className="mb-5">
                ...
                {getExpenseOptionsForSettlement(settlementModal).map(expense => {
                  ...
                  return (
                    <label key={expense._id} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition ${isSelected ? 'border-emerald-500 bg-emerald-500/10' : 'border-gray-700 bg-gray-800 hover:bg-gray-700'}`}>
                      <input type="checkbox" checked={isSelected} onChange={e => { if (e.target.checked) setSettlementExpenseIds([...settlementExpenseIds, expense._id]); else setSettlementExpenseIds(settlementExpenseIds.filter(id => id !== expense._id)); }} className="w-4 h-4 rounded border-gray-600 text-emerald-500 focus:ring-emerald-500 bg-gray-700" />
                      <span className="text-sm font-medium">{expense.description} - ₹{Number(expense.amount || 0).toFixed(2)}</span>
                    </label>
                  );
                })}
                {getExpenseOptionsForSettlement(settlementModal).length === 0 && <p className="text-sm text-gray-400">No related expenses found.</p>}
              </div>
            </div>
            <div className="space-y-3">
              <button onClick={handleUpdateSettlement} disabled={savingSettlement} className="w-full bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50 text-white py-3 rounded-xl font-semibold transition">{savingSettlement ? "Saving..." : "Save changes"}</button>
              <button onClick={handleDeleteSettlement} disabled={savingSettlement} className="w-full bg-red-500 hover:bg-red-600 disabled:opacity-50 text-white py-3 rounded-xl font-semibold transition">Delete payment</button>
              <button onClick={() => { setSettlementModal(null); setSettlementAmount(""); setSettlementExpenseIds([]); }} disabled={savingSettlement} className="w-full bg-gray-800 hover:bg-gray-700 disabled:opacity-50 text-white py-3 rounded-xl transition">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 sm:p-6 w-full max-w-sm">
            <div className="text-center mb-6">
              <div className="w-14 h-14 bg-red-500/10 rounded-full flex items-center justify-center mx-auto mb-4"><FiTrash2 className="text-red-400" size={24} /></div>
              <h2 className="text-xl font-bold">Delete Expense?</h2>
              <p className="text-gray-400 text-sm mt-2">Delete <span className="text-white font-medium">"{deleteModal.description}"</span>? This cannot be undone.</p>
            </div>
