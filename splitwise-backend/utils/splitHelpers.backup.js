...
async function calculateUserReceivables(groupId, userId) {
  ...
  const settlements = await Settlement.find({ group: groupId })
    ...
    .sort({ date: -1 });
  const balances = {};
  const ensureBalance = person => {
    const id = person?._id?.toString() || person?.toString();
    if (!id || id === currentUserId) return null;
    if (!balances[id]) {
      balances[id] = { memberId: id, name: person.name || 'Member', email: person.email, amount: 0 };
    }
    return balances[id];
  };
  group?.members?.forEach(member => ensureBalance(member));
  const addAmount = (person, amount) => {
    const b = ensureBalance(person);
    if (b) b.amount = +(b.amount + amount).toFixed(2);
  };
  expenses.forEach(expense => {
    let currentUserPaidAmount = 0;
    if (expense.paidByMultiple?.length > 0) {
    ...
    const userSplit = expense.splits.find(s => (s.user?._id?.toString() || s.user?.toString()) === currentUserId);
    const netAmount = currentUserPaidAmount - (userSplit ? userSplit.amount : 0);
    if (netAmount > 0) {
      expense.splits.forEach(split => {
        const splitUserId = split.user?._id?.toString() || split.user?.toString();
        if (splitUserId && splitUserId !== currentUserId) {
          addAmount(split.user, -(split.amount * (currentUserPaidAmount / expense.amount)));
        }
      });
    } else if (netAmount < 0) {
      const amountOwed = Math.abs(netAmount);
      if (expense.paidByMultiple?.length > 0) {
        expense.paidByMultiple.forEach(payer => {
          if (payer.user?._id.toString() !== currentUserId) {
            addAmount(payer.user, amountOwed * (payer.amount / expense.amount));
          }
        });
      } else if (expense.paidBy) {
        addAmount(expense.paidBy, amountOwed);
      }
    }
  });
  settlements.forEach(settlement => {
    const payerId = settlement.paidBy._id.toString();
    const payeeId = settlement.paidTo._id.toString();
    if (payerId === currentUserId) addAmount(settlement.paidTo, -settlement.amount);
    else if (payeeId === currentUserId) addAmount(settlement.paidBy, settlement.amount);
  });
  return Object.values(balances)
    .filter(b => b.amount < -0.009)
    .map(b => ({ ...b, amount: +Math.abs(b.amount).toFixed(2) }))
